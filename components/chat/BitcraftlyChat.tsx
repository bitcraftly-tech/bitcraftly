...
const QUICK_DEFAULT: QuickReply[] = [
  { label: "💰 Website Cost", value: "pricing" },
...
export default function BitcraftlyChat() {
  ...
  useEffect(() => {
    const onOpenChat = (event: Event) => {
      ...
      setOpen(true);
      if (detail?.query) {
        window.setTimeout(() => sendMessageRef.current(detail.query), 400);
      }
    };
    window.addEventListener("bc-open-chat", onOpenChat);
    return () => window.removeEventListener("bc-open-chat", onOpenChat);
  }, []);
        ...
        <div className="bc-chat-body">
          {msgs.length === 0 && !thinking && (
            <div className="bc-chat-welcome">
              ...
              <p>Hi! I&apos;m BitBot 👋 How can I help you today?</p>
              <div className="bc-chat-quick bc-chat-quick--welcome">
                {QUICK_DEFAULT.map((q) => (
