...
export default function SchoolFooter() {
  ...
  const goLink = (label: string) => {
    ...
    if (id === "enquiry-form") document.getElementById("enquiry-form")?.scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  return (
    <footer className="school-bg-footer text-white">
      <div className={`${CONTAINER} grid gap-10 py-14 md:grid-cols-2 lg:grid-cols-5`}>
        <div className="lg:col-span-1">
          ...
          <p className="mt-4 text-sm leading-relaxed text-white/75">
            {SCHOOL_FULL_NAME} — CBSE affiliated co-educational school delivering holistic learning since 1999. Fictional demo by
            Bitcraftly.
          </p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm text-white/75">
            {FOOTER_USEFUL.map((l) => (
              <li key={l.label}>
                <button type="button" onClick={() => handleUsefulLink(l.action)} className="hover:text-white">
                  {l.label}
                </button>
              </li>
            ))}
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-white/70">Subscribe for campus updates and admission alerts.</p>
          <form
          ...
          >
            ...
            />
            <button type="submit" className="school-btn-orange px-4 text-lg font-bold" aria-label="Subscribe">
              →
            </button>
          </form>
        </div>
      ...
      <div className="border-t border-white/10 py-5">
        <div className={`${CONTAINER} flex flex-col items-center justify-between gap-3 text-center text-xs text-white/55 sm:flex-row`}>
          <p>
            © {new Date().getFullYear()} {SCHOOL_FULL_NAME}. All rights reserved. · {formatVisitorCount(visitorCount)} visitors
          </p>
          <div className="flex gap-4">
            <button type="button" onClick={() => showToast("Privacy policy · demo")} className="hover:text-white">
              Privacy Policy
            </button>
            <button type="button" onClick={() => showToast("Terms · demo")} className="hover:text-white">
              Terms & Conditions
            </button>
          </div>
          <p>
            Built by{" "}
            <Link href="https://bitcraftly.com/" className="text-[var(--school-orange)] hover:underline" {...newTabProps("https://bitcraftly.com/")}>
              Bitcraftly
            </Link>
          </p>
        </div>
