import { SCHOOL_FULL_NAME, SCHOOL_TAGLINE } from "@/app/portfolio/school-website-showcase/school-demo-data";
...
export default function SchoolLogo({ className = "", size = "md", variant = "default" }: SchoolLogoProps) {
  const s = sizes[size];
  ...
  return (
    <span className={`inline-flex items-center gap-3 ${className}`}>
      ...
      <span className="flex min-w-0 flex-col leading-tight">
        ...
        <span className={`${s.sub} font-medium italic ${light ? "text-white/70" : "text-[var(--school-muted)]"}`}>{SCHOOL_TAGLINE}</span>
      </span>
    </span>
  );
}
