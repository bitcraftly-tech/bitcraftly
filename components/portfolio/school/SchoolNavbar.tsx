  ...
  return (
    <header className="sticky top-0 z-50">
      <div className="school-top-bar text-xs text-white">
        <div className={`${CONTAINER} flex flex-wrap items-center justify-between gap-2 py-2`}>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <a href={`mailto:${SCHOOL_EMAIL}`} className="inline-flex items-center gap-1.5 hover:text-[var(--school-orange)]">
              <Mail className="h-3.5 w-3.5" aria-hidden />
              <span className="hidden sm:inline">{SCHOOL_EMAIL}</span>
            </a>
            <a href={`tel:${SCHOOL_PHONE}`} className="inline-flex items-center gap-1.5 hover:text-[var(--school-orange)]">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {LOGINS.map((l) => (
              <button key={l} type="button" onClick={() => showToast(`${l} · demo portal`)} className="hover:text-[var(--school-orange)]">
                {l}
              </button>
            ))}
      ...
      <div className="school-bg-card border-b school-border shadow-sm">
        <div className={`${CONTAINER} grid grid-cols-[auto_1fr_auto] items-center gap-2 py-3 lg:gap-4`}>
              ...
              {NAV.map((item) => (
                <button
                  ...
                  className={`whitespace-nowrap px-1.5 py-1 text-xs font-semibold transition lg:text-sm ${
                    activeNav === item.label ? "text-[var(--school-orange)]" : "text-[var(--school-text)] hover:text-[var(--school-orange)]"
                  }`}
                >
                  {item.label}
                </button>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button type="button" className="rounded-md p-2 xl:hidden" onClick={() => setMenuOpen((o) => !o)} aria-label="Menu">
        ...
        {menuOpen ? (
          <div className="school-border border-t px-4 py-4 xl:hidden">
            <div className="flex flex-col gap-1">
              {NAV.map((item) => (
                <button key={`m-${item.label}`} type="button" className="py-2 text-left text-sm font-medium" onClick={() => go(item.id, item.label)}>
                  {item.label}
                </button>
              ))}
              <button type="button" onClick={scrollToEnquiry} className="school-btn-primary mt-3 rounded-md py-2.5 text-sm font-bold uppercase">
                Enquire Now
              </button>
            </div>
          </div>
