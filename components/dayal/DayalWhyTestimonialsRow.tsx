          ...
          <DayalReveal className="flex min-h-0 flex-col">
            ...
            <div className="mt-6 flex min-h-0 flex-1 flex-col gap-5 lg:mt-7">
              <ul className="shrink-0 space-y-2.5 rounded-xl border border-[#0b1633]/8 bg-[#fffdf9] p-4 sm:p-5">
                {WHY_TRUST.map((point) => (
                  <li key={point} className="flex items-start gap-3 text-[#0b1633]">
                    ...
                    <span className="text-sm font-medium leading-snug sm:text-[0.95rem]">{point}</span>
                  </li>
                ))}
              </ul>

              <div className="relative min-h-[12rem] flex-1 overflow-hidden rounded-xl shadow-[0_12px_40px_rgba(11,22,51,0.12)] ring-1 ring-[#0b1633]/10 sm:min-h-[14rem] lg:min-h-[16rem]">
                <Image
                    ...
                    {TESTIMONIALS.map((_, i) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all duration-300 ${
                          i === index ? "w-6 bg-[#c8a46b]" : "w-1.5 bg-[#0b1633]/20 hover:bg-[#0b1633]/35"
                        }`}
