...
const FEATURES = [
  {
    ...
    desc: 'Morning mandi pickups · traceable poultry · seasonal garnish rotations.',
    icon: Leaf,
  },
  {
    title: 'Expert chefs',
    desc: 'Open‑flame tandoors · continental brigades · pastry counter artistry.',
    icon: ChefHat,
  },
  {
    title: 'Hygienic kitchen',
    desc: 'Temperature logs · FSSAI playbook overlays · glove‑colour zoning cues.',
    icon: ShieldCheck,
  },
  {
    title: 'Fast delivery',
    desc: 'Thermal‑lined runners · ETA honesty banners · spill‑proof packaging lab.',
    icon: Bike,
  },
  {
    title: 'Cozy ambience',
    desc: 'Acoustic dampened bays · amber sconces · vinyl‑quiet playlists nightly.',
    icon: Armchair,
  },
] as const;
...
const MENU_POPULAR = [
  {
    dish: 'Zaika butter chicken bowl',
    detail: 'Basmati pilaf · laccha onion · naan shard',
    price: '₹420',
  },
  {
    dish: 'Harissa paneer tikka',
    detail: 'Hung yogurt marinade · mint chutney lattice',
    price: '₹340',
  },
  {
    dish: 'Charred corn ribs · lime kosho',
    detail: 'Vegan · sticky glaze · sesame crunch',
    price: '₹280',
  },
  {
    dish: 'Ocean broth ramen bowl',
    detail: 'Bonito dashi · nori crackle · jammy egg swap optional',
    price: '₹460',
  },
  {
    dish: 'Bittersweet chocolate dome',
    detail: 'Orange zest crémeux · hazelnut soil',
    price: '₹320',
  },
] as const;
...
const GALLERY = [
  { caption: 'Char station · live flames', tone: 'from-orange-500/35 to-neutral-950' },
  { caption: 'Chef plating finale', tone: 'from-violet-600/30 to-slate-100' },
  { caption: 'Private booth lighting rig', tone: 'from-amber-600/25 to-violet-950' },
  { caption: 'Dessert mirror glaze pour', tone: 'from-rose-600/30 to-slate-100' },
  { caption: 'Wine wall vignette', tone: 'from-purple-700/28 to-slate-100' },
  { caption: 'Courtyard alfresco night', tone: 'from-teal-700/20 to-orange-950/40' },
] as const;
    ...
    <>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-14">
            <div>
              ...
              <p className="mt-5 max-w-xl text-base leading-relaxed text-dark-text-secondary sm:text-lg">
                A cinematic dining landing specimen — menu storytelling, reservation friction drops,
                and delivery urgency strips tuned for conversion without breaking brand warmth.
              </p>
                ...
                <div className="relative aspect-[5/4] p-6 sm:p-8">
                  ...
                  <div className="relative flex h-full flex-col justify-between rounded-2xl border border-slate-200/80 bg-white/80 p-5 backdrop-blur-sm">
                    ...
                    <div className="mx-auto flex max-w-[85%] flex-col items-center">
                      ...
                      <p className="mt-4 text-center font-[var(--font-playfair)] text-lg font-semibold text-white sm:text-xl">
                        Heritage spice dome · hero dish
                      </p>
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-orange-400/90">
              Featured dishes
            </p>
            <h2 className="mt-2 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">
              Plate stories worth scrolling
            </h2>
          </div>
          <p className="max-w-md text-sm text-dark-text-secondary">
            Large-format tiles mimic editorial menu UX — macro gradients evoke hero photography
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {FEATURED.map((f) => (
            <article
              key={f.name}
              className={`overflow-hidden rounded-2xl border border-dark-border-primary bg-gradient-to-br shadow-xl ring-1 ring-orange-500/10 ${f.tone}`}
            >
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-orange-400/85">
              Chef specials
            </p>
            <h2 className="mt-2 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">
              Limited timelines
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {CHEF_SPECIALS.map((s) => (
            <div key={s.title} className={`rounded-2xl border px-6 py-6 ${s.tone}`}>
              ...
              <p className="mt-4 text-[11px] font-semibold uppercase tracking-wide text-violet-800">
                {s.meta}
              </p>
            </div>
          ))}
      ...
      <section className="border-y border-dark-border-primary bg-dark-bg-primary py-14 md:py-16">
        <div className={`${CONTAINER}`}>
          ...
          <div className="mt-10 grid min-w-0 grid-cols-2 gap-3 md:grid-cols-3 lg:gap-4">
            {GALLERY.map((g, i) => (
              <div
                key={g.caption}
                className={`relative min-w-0 overflow-hidden rounded-xl border border-dark-border-primary bg-gradient-to-br ${g.tone} ${
      ...
      <section
        id="reservation"
        className="scroll-mt-28 border-y border-dark-border-primary bg-dark-bg-secondary/35 py-14 md:py-16"
      >
        <div className={`${CONTAINER}`}>
          <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
            <div>
              ...
              <p className="mt-4 text-sm leading-relaxed text-dark-text-secondary">
                Deposit flows & WhatsApp confirmations wire up on production builds — this canvas
                captures premium visual hierarchy only.
              </p>
              <div className="mt-8 rounded-xl border border-dark-border-primary bg-dark-bg-card p-5">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-dark-text-tertiary">
                  Concierge desk
                </p>
                <ShowcaseAnchor
                  href={`tel:${SUPPORT_PHONE_DISPLAY.replace(/\s/g, '')}`}
                  className="mt-2 block text-lg font-semibold text-orange-300 hover:text-orange-800"
                >
                  {SUPPORT_PHONE_DISPLAY}
                </ShowcaseAnchor>
                <p className="mt-2 text-xs text-dark-text-secondary">
                  Callbacks within opening hours · fictional SLA.
                </p>
              </div>
            </div>
      ...
      <section className={`${CONTAINER} pb-16 md:pb-20`}>
        <div className="relative overflow-hidden rounded-2xl border border-orange-500/35 bg-gradient-to-r from-orange-950/90 via-[#140818] to-violet-950 px-8 py-10 md:px-12 md:py-12">
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-orange-800/90">
                Food delivery
              </p>
              <h3 className="mt-2 font-[var(--font-playfair)] text-2xl font-semibold text-white md:text-3xl">
                Thermal‑lined runners · 35‑minute honesty ETA
              </h3>
              <p className="mt-3 max-w-xl text-sm leading-relaxed text-orange-100/75">
                Banner strip mirrors conversion-heavy delivery pushes — swap aggregator badges &
                promo timers when you go live.
              </p>
            </div>
            <ShowcaseLink
        ...
        <p className="mx-auto mt-8 max-w-2xl text-center text-[11px] leading-relaxed text-dark-text-tertiary">
          Ember & Vine / Zaika Kitchen naming cues are fictional UI specimens only · menu pricing
          illustrative · crafted by Bitcraftly.
        </p>
      </section>
    </>
  );
