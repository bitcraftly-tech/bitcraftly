...
import { Award, Calendar, Gem, GraduationCap, MapPin, Monitor } from 'lucide-react';
...
type HiWStep =
  (typeof RPYTECH_HOW_IT_WORKS.left)[number] | (typeof RPYTECH_HOW_IT_WORKS.right)[number];
        ...
        <div className="rpytech-hiw-layout">
          <div className="rpytech-hiw-col rpytech-hiw-col--left">
            {RPYTECH_HOW_IT_WORKS.left.map((step, index) => (
              <HowItWorksStep key={step.title} step={step} side="left" delay={120 + index * 90} />
            ))}
          </div>
          ...
          <div className="rpytech-hiw-col rpytech-hiw-col--right">
            {RPYTECH_HOW_IT_WORKS.right.map((step, index) => (
              <HowItWorksStep key={step.title} step={step} side="right" delay={150 + index * 90} />
            ))}
          </div>
        </div>
      </div>
