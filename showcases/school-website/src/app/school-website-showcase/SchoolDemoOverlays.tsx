  ...
  principal: {
    ...
    body: (
      <p className="school-text-muted text-sm leading-relaxed">
        Dear parents and students — at {SCHOOL_FULL_NAME} we believe in roots with wings:
        value-based grounding and the confidence to excel globally. Our faculty, house system and
        modern labs work together for holistic growth. Welcome to our campus family.
      </p>
    ),
  },
  history: {
    ...
    body: (
      <p className="school-text-muted text-sm leading-relaxed">
        {SCHOOL_SOCIETY} registered in 1965 laid the foundation. The school was established in 1973
        and has since become a leading institution in East Delhi — CBSE affiliated, Delhi Govt.
        recognized, co-educational excellence.
      </p>
    ),
  },
  sports: {
...
function DemoForm({ type, onClose }: { type: SchoolFormType; onClose: () => void }) {
  ...
  return (
    <ModalShell title={FORM_TITLES[type]} onClose={onClose}>
      <p className="school-text-muted text-sm">
        Demo form · no data stored · production connects to your CRM.
      </p>
      ...
      <button
        type="button"
        className="school-btn-primary mt-5 w-full rounded-md py-2.5 text-sm font-bold uppercase"
        onClick={submit}
      >
        Submit
      </button>
  ...
  return (
    <ModalShell title={item.title} onClose={onClose} wide>
      <SchoolLazyImage
        ...
        wrapperClassName="aspect-[4/3] w-full rounded-lg sm:aspect-video"
        eager
      />
    </ModalShell>
  );
...
export function SchoolDemoOverlays() {
  ...
  return (
    <>
      ...
      {galleryIndex !== null ? (
        <GalleryModal index={galleryIndex} onClose={() => setGalleryIndex(null)} />
      ) : null}
