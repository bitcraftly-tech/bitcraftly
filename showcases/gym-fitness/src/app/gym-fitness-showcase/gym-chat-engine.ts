import { GYM_CENTERS, MEMBERSHIP_PASSES, TRAINERS, WORKOUT_FORMATS } from './gym-demo-data';
...
function listFormats(filter?: string) {
  ...
  return items.map((f) => `• ${f.name} (${f.duration}) — ${f.tagline} · ${f.calories}`).join('\n');
}
...
export function answerFitRallyQuery(
  ...
  if (captured) {
    ...
    const introOnly =
      ...
      /^(my\s+name\s+is|i\s*am|i'?m|this\s+is|call\s+me|mera\s+naam|main\s+|mein\s+)/i.test(query);
  ...
  if (
    /(center|gym|location|near|branch|saket|connaught|delhi|mumbai|bengaluru|hyderabad|kolkata)/.test(
      lower,
    )
  ) {
