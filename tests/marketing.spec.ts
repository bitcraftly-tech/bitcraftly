...
import { expectMarketingPage, MARKETING_PAGES } from './helpers/marketing';
...
for (const pageSpec of MARKETING_PAGES) {
  test.describe(`${pageSpec.name} page`, () => {
    test(`loads with hero, CTA, and no console errors (${pageSpec.path})`, async ({ page }) => {
...
test.describe('Primary navigation', () => {
  test('header mega-menu reaches key marketing routes', async ({ page }, testInfo) => {
    test.skip(
