  ...
  for (const width of MOBILE_WIDTHS) {
    test(`no horizontal overflow at ${width}px`, async ({ page }) => {
      ...
      if (layout.hamburgerRight !== null) {
        expect(width - layout.hamburgerRight).toBeGreaterThanOrEqual(expectedPadding - 1);
      }

      if (layout.heroPaddingLeft !== null) {
        expect(Math.abs(layout.paddingLeft - layout.heroPaddingLeft)).toBeLessThan(2);
      }
      ...
      if (hamburgerBeforeClose && hamburgerAfterClose) {
        expect(Math.abs(hamburgerBeforeClose.x - hamburgerAfterClose.x)).toBeLessThan(2);
        expect(Math.abs(hamburgerBeforeClose.y - hamburgerAfterClose.y)).toBeLessThan(2);
      }
