...
const variantStyles: Record<IconBoxVariant, string> = {
  default: 'border border-border bg-surface text-foreground shadow-sm',
  primary:
    'border border-transparent bg-gradient-to-br from-primary to-accent text-primary-foreground shadow-md',
  secondary: 'border border-transparent bg-secondary text-secondary-foreground shadow-sm',
  glass: 'border border-border/80 bg-background/70 text-foreground shadow-sm backdrop-blur-md',
};
...
export function IconBox({
  ...
  if (!children) {
    return <div className={cn('inline-flex', className)}>{iconContainer}</div>;
  }

  return (
    <div className={cn('inline-flex items-center gap-[var(--space-2)]', className)}>
      {iconContainer}
      <div>{children}</div>
    </div>
  );
