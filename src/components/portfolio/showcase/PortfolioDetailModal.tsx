        ...
        >
          ...
          >
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-[#e8ecef] px-5 py-3.5 dark:border-dark-border-primary sm:px-6">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#9b59b6] dark:text-indigo-400">
                Case Study Overview
              </p>
              <button
                type="button"
                ...
                <div className="flex min-w-0 gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h2
                      ...
                      className="mt-2 text-xl font-bold text-[#2c3e50] dark:text-dark-text-primary sm:text-2xl"
                    >
                      {project.title}
                    </h2>
                    <p className="mt-2 text-sm leading-relaxed text-[#7f8c8d] dark:text-dark-text-secondary">
                      {project.cardLine}
                    </p>
                ...
                <div className="mt-5 grid min-w-0 grid-cols-3 gap-2 sm:mt-6 sm:gap-3">
                  {caseStudyHeadlineMetrics(project).map((m) => (
                    ...
                    >
                      ...
                      <p className="mt-2 w-full truncate text-sm font-bold text-[#2c3e50] dark:text-dark-text-primary sm:text-base">
                        {m.value}
                      </p>
                      <p className="mt-0.5 w-full truncate text-[10px] font-medium text-[#95a5a6] dark:text-dark-text-tertiary">
                        {m.label}
                      </p>
                    </div>
                  ))}
              ...
              <div className="grid gap-6 px-5 py-5 sm:px-6 sm:py-6 lg:grid-cols-[1.35fr_1fr] lg:gap-8">
                <div className="space-y-6">
                  <section>
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-[#7f8c8d] dark:text-dark-text-secondary">
                  ...
                  <section>
                    ...
                    <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                      {caseStudyKeyFeatures(project).map((f) => (
                        <li
                          key={f}
                          className="flex items-start gap-2 text-sm text-[#7f8c8d] dark:text-dark-text-secondary"
                        >
                          <span
                ...
                <aside className="h-fit rounded-2xl border border-[#e8ecef] bg-[#f4f6f8]/80 p-4 dark:border-dark-border-primary dark:bg-dark-bg-secondary/80 sm:p-5">
                  <dl className="space-y-4">
                    ...
                    <MetaRow
                      icon={FolderKanban}
                      label="Category"
                      value={caseStudyCategoryLabel(project)}
                    />
                    <MetaRow
                      icon={Wrench}
                      label="Services"
                      value={caseStudyServices(project).join(', ')}
                    />
                  </dl>
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2 border-t border-[#e8ecef] bg-white px-4 py-3.5 dark:border-dark-border-primary dark:bg-dark-bg-card sm:gap-3 sm:px-5">
              ...
              <Link
                href="/contact?intent=consultation&source=portfolio-modal"
              ...
              <Link
                href={project.caseStudyHref}
                className={`${PS_BTN_GHOST} sm:ml-auto`}
                onClick={onClose}
              >
                Full page →
              </Link>
...
function MetaRow({
  ...
  return (
    <div className="flex gap-3">
      ...
      <div>
        <dt className="text-[11px] font-semibold uppercase tracking-wide text-[#95a5a6] dark:text-dark-text-tertiary">
          {label}
        </dt>
        <dd className="mt-0.5 text-sm font-medium text-[#2c3e50] dark:text-dark-text-primary">
          {value}
        </dd>
      </div>
    </div>
