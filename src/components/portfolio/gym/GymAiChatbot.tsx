...
import {
  ...
  type ChatMemory,
  type ChatMessage,
} from '@/app/portfolio/gym-fitness-showcase/gym-chat-engine';
...
export default function GymAiChatbot() {
  ...
  const sendChatRecordToWhatsApp = useCallback(
    (reason: 'close' | 'reset') => {
      ...
      const message = formatChatTranscriptForWhatsApp(
        snapshot,
        reason,
        city,
        memoryRef.current,
  ...
  const send = (raw: string) => {
    ...
    replyTimerRef.current = window.setTimeout(
      () => {
        const { reply, memory: nextMemory } = answerFitRallyQuery(
          text,
          city,
          memoryRef.current,
        );
  ...
  return (
    <>
      ...
      >
        {open ? <X className="gym-chat-fab__icon" aria-hidden /> : <ChatRobotIcon className="gym-chat-fab__robot" />}
      </button>
          ...
          <div ref={listRef} className="gym-chat-panel__body">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={
                  msg.role === 'bot' ? 'gym-chat-row gym-chat-row--bot' : 'gym-chat-row gym-chat-row--user'
                }
              >
                {msg.role === 'bot' ? (
            ...
            >
              ...
              <input
                ...
                placeholder={
                  memory.visitorName
                    ? `Ask anything, ${memory.visitorName}…`
                    : 'Enter your name…'
                }
                autoComplete="off"
                disabled={busy}
