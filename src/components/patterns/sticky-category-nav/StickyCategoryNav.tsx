...
export function StickyCategoryNav({ groups, ariaLabel, className }: StickyCategoryNavProps) {
  const navId = useId();
  ...
  const scrollToGroup = useCallback((id: string) => {
    ...
    const top = el.getBoundingClientRect().top + window.scrollY - HEADER_OFFSET - 56;
            ...
            return (
              <a
                ...
                className={cn(
                  ...
                  isActive ? 'services-category-tab--active' : 'services-category-tab--idle',
                )}
              >
                {group.title}
              </a>
