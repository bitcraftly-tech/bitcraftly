...
import type { SectionHeaderAlign, SectionHeaderMaxWidth, SectionHeaderProps } from './types';
  ...
  return (
    ...
    >
      <div
        className={cn(
          'flex w-full flex-col gap-[var(--space-3)] md:gap-[var(--space-4)]',
          hasActions && 'sm:flex-row sm:items-end sm:justify-between sm:gap-[var(--space-4)]',
      ...
      >
        ...
        >
          {eyebrow ? (
            <Badge variant="primary" size="sm" className="uppercase tracking-[0.15em]">
              {eyebrow}
            </Badge>
          ) : null}
          ...
          {description ? (
            <Text id={descriptionId} size="lg" muted className="text-pretty leading-relaxed">
              {description}
            </Text>
          ) : null}
        </div>
        ...
        {children ? (
          <div className={cn('shrink-0', align === 'center' && 'flex justify-center')}>
            {children}
          </div>
        ) : null}
      </div>
