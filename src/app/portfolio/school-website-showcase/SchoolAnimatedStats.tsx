...
function AnimatedStat({
  ...
  icon,
  embedded,
}: (typeof STATS_BAR)[number] & { embedded?: boolean }) {
  ...
  return (
    <div ref={ref} className="flex items-center justify-center gap-3 px-3 py-5 sm:px-5 lg:py-6">
      <Icon
        className="h-7 w-7 shrink-0 text-[var(--school-orange)] sm:h-8 sm:w-8"
        strokeWidth={1.5}
        aria-hidden
      />
...
export default function SchoolAnimatedStats({
  embedded = false,
  floating = false,
}: SchoolAnimatedStatsProps) {
  const grid = (
  ...
  if (embedded) {
    return <div className="school-hero-stats-panel w-full px-1 pb-0 pt-1 sm:px-2">{grid}</div>;
  }
