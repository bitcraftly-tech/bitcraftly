...
import { Grid3x3, Layers, Pause, Play, Sparkles, Volume2, VolumeX } from 'lucide-react';
...
const TRENDING = [
  ...
  },
  {
    title: 'Quiet Signals',
    meta: 'Limited series',
    gradient: 'from-indigo-500 via-violet-700 to-zinc-950',
  },
  {
    title: 'Solar Drift',
    meta: 'Sci‑Fi',
    gradient: 'from-amber-500/90 via-orange-700 to-purple-950',
  },
  {
    title: 'Velvet Circuit',
    meta: 'Crime drama',
    gradient: 'from-rose-600 via-purple-800 to-neutral-950',
  },
  {
    title: 'Echo Basin',
    meta: 'Documentary',
    gradient: 'from-teal-600 via-blue-800 to-violet-950',
  },
] as const;

const CONTINUE = [
  {
    title: 'Crimson Line · Ep 4',
    pct: 72,
    gradient: 'from-red-900/80 via-purple-900 to-slate-100',
  },
  {
    title: 'Midtown Nights · Ep 2',
    pct: 38,
    gradient: 'from-slate-700 via-violet-900 to-slate-100',
  },
  {
    title: 'Atlas Rising · 58%',
    pct: 58,
    gradient: 'from-blue-900/70 via-indigo-900 to-slate-100',
  },
] as const;
    ...
    <div className="group cursor-default overflow-hidden rounded-lg border border-dark-border-primary bg-dark-bg-secondary shadow-lg transition hover:border-violet-500/45 hover:shadow-violet-500/10">
      <div className={`relative aspect-[2/3] bg-gradient-to-br ${gradient}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent p-3 pt-12">
          ...
          <p className="mt-0.5 font-[var(--font-playfair)] text-sm font-semibold text-white">
            {title}
          </p>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-white/5 px-4 py-2.5">
        ...
        <span className="ml-2 text-[10px] font-medium tracking-wide text-white/35">
      ...
      <div className="grid gap-0 lg:grid-cols-[1fr_220px]">
          ...
          <div className="relative flex h-full flex-col justify-end p-4 sm:p-6 lg:p-8">
            <div className="max-w-xl space-y-2">
              ...
              <p className="max-w-md text-xs leading-relaxed text-white/65 sm:text-sm">
                Fictional title · Demonstrates layered gradients, typography hierarchy, and
                in-player branding zones — no third‑party artwork.
              </p>
            </div>
        ...
        <aside className="border-t border-white/5 bg-[#0b0b12] lg:border-l lg:border-t-0">
          <div className="border-b border-white/5 px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-300/85">
              Continue watching
            </p>
          </div>
          <div className="space-y-3 p-3">
            {CONTINUE.map((row) => (
              <div
                key={row.title}
                className="overflow-hidden rounded-lg border border-white/8 bg-white/[0.03]"
              >
                ...
                <p className="px-2 py-2 text-[11px] font-medium leading-snug text-white/80">
                  {row.title}
                </p>
              </div>
            ))}
      ...
      <div className="border-t border-white/5 px-4 py-3">
        <p className="text-[10px] text-white/35">
          Mock player only · Posters and titles are fictional showcase assets · Not affiliated with
          any streaming brand.
        </p>
      </div>
    </div>
        ...
        <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.85fr)] lg:gap-14">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-relaxed text-dark-text-secondary sm:text-lg">
              ...
              <span className="font-medium text-dark-text-primary">CSS Grid</span> — structured like
              a real product engineering deliverable.
            </p>
          ...
          <div className="rounded-2xl border border-dark-border-primary bg-gradient-to-br from-violet-50/40 via-dark-bg-card to-dark-bg-secondary p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
            <div className="rounded-[15px] border border-white/5 bg-dark-bg-primary/80 p-4 sm:p-5">
              ...
              <p className="mt-3 text-sm leading-relaxed text-dark-text-secondary">
                Rails, hero spotlight, and sidebar recall patterns common to premium OTT surfaces —
                implemented as a lean React surface so motion, accessibility, and layout stay
                maintainable.
              </p>
              <div className="mt-5 grid grid-cols-3 gap-2 sm:gap-3">
                {TRENDING.slice(0, 3).map((m) => (
                  <div
                    key={m.title}
                    className="overflow-hidden rounded-md border border-slate-200/80"
                  >
      ...
      <section id="browse" className={`${CONTAINER} scroll-mt-28 pb-14 md:pb-16`}>
        <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-md text-sm text-dark-text-secondary">
            Custom chrome over familiar metaphors — transport controls, progress scrubbing lane, and
            editorial rails.
          </p>
        </div>
        <OttPreviewMock />
      ...
      <section className="border-t border-dark-border-primary bg-dark-bg-secondary/30 py-14 md:py-16">
        <div className={`${CONTAINER}`}>
          <div className="mb-10 max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-relaxed text-dark-text-secondary">
              Cards mirror how we talk about real engagements — integration boundaries, layout
              discipline, and theming.
            </p>
          </div>
      ...
      <section className={`${CONTAINER} pb-14 md:pb-20`}>
        <div className="rounded-2xl border border-dark-border-primary bg-dark-bg-card p-8 md:p-10">
          ...
          <p className="mt-5 max-w-3xl text-sm leading-relaxed text-dark-text-secondary md:text-base">
            This page is a UI specimen for Bitcraftly visitors: it demonstrates how we structure
            dense media layouts, streaming-adjacent UX, and branded dark interfaces without relying
            on third‑party key art. Stack labels reflect typical tooling we compose on client
            engagements — React for UI, HLS where manifests matter, Video.js when we want proven
            player primitives, CSS Grid for responsive rails, and SCSS when tokenized themes need to
            scale.
          </p>
        </div>
      </section>
      ...
      <section id="plans" className={`${CONTAINER} scroll-mt-28 pb-16 md:pb-24`}>
        <div className="relative overflow-hidden rounded-2xl border border-violet-300 bg-gradient-to-r from-violet-100 via-fuchsia-50 to-indigo-100 px-8 py-10 md:px-12 md:py-12">
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="font-[var(--font-playfair)] text-2xl font-semibold text-white md:text-3xl">
                Want Similar Solution?
              </h2>
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-violet-100/80">
                Share playback goals, audience devices, and release milestones — we&apos;ll map
                architecture and UI debt before a single sprint burns.
              </p>
            </div>
            <ShowcaseLink
