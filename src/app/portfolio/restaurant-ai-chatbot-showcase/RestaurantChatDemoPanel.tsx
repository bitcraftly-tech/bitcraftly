          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-emerald-300/90">
                Online · avg reply &lt; 12s · fictional demo
              </p>
            </div>
          </div>
                ...
                msg.role === 'bot' ? (
                  ...
                  <div key={i} className="flex justify-end">
                    <div className="max-w-[88%] rounded-2xl rounded-tr-md border border-slate-200/80 bg-white/[0.07] px-4 py-3 text-sm leading-relaxed text-white">
                      {msg.text}
                      <p className="mt-2 text-right text-[10px] font-medium text-white/35">
                        {msg.time}
                      </p>
                    </div>
                  </div>
          ...
          <aside className="bg-[#05040d] p-4">
            ...
            <ul className="mt-4 space-y-3">
              {DISH_SUGGESTIONS.map((d) => (
                <li
                  key={d.name}
                  className="rounded-lg border border-slate-200/80 bg-white/[0.04] p-3"
                >
                  <p className="text-xs font-semibold text-white">{d.name}</p>
                  <p className="mt-1 text-[10px] leading-snug text-white/50">{d.tag}</p>
                  <p className="mt-2 font-mono text-[11px] font-semibold text-emerald-300/90">
                    {d.price}
                  </p>
                </li>
              ))}
            ...
            <p className="mt-6 text-[10px] leading-relaxed text-white/35">
              Sidebar mirrors POS-linked recommendation rails — hero SKUs rotate by inventory &
              margin rules.
            </p>
          </aside>
        </div>
