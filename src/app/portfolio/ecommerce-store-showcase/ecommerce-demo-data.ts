...
export type PriceBand = 'all' | 'under-1000' | '1000-5000' | '5000-plus';
...
export const CATEGORY_TILES = [
  {
    label: 'Home & Kitchen',
    blurb: 'Essentials from ₹699',
    department: 'Home & Kitchen' as ShopDepartment,
    image: img('photo-1556912173-46c336c7fd55', 600, 480),
  },
  {
    label: 'Electronics',
    blurb: 'Phones, audio & more',
    department: 'Electronics' as ShopDepartment,
    image: img('photo-1468495244123-6c6c332eeece', 600, 480),
  },
  {
    label: 'Fashion',
    blurb: 'New season styles',
    department: 'Fashion' as ShopDepartment,
    image: img('photo-1445205170230-053b83016050', 600, 480),
  },
  {
    label: 'Deals',
    blurb: 'Up to 60% off',
    department: 'Deals' as ShopDepartment,
    image: img('photo-1607082348824-0a96f2a4b9da', 600, 480),
  },
] as const;

/** Wide lifestyle banner for the storefront hero */
export const HERO_BANNER_IMAGE = img('photo-1483985988355-763728e1935b', 1400, 900);
...
export function productMatchesSearch(product: ShopProduct, rawQuery: string) {
  ...
  if (!q) return true;
  const haystack =
    `${product.title} ${product.keywords.join(' ')} ${product.department}`.toLowerCase();
