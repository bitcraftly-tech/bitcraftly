...
function GirlBotAvatar({ size = 36, className = '' }: { size?: number; className?: string }) {
  return (
    ...
    >
      ...
      {/* Hair */}
      <path
        d="M20 26 Q20 14 40 14 Q60 14 60 26 L60 22 Q60 10 40 10 Q20 10 20 22 Z"
        fill="#2d3a4a"
      />
      {/* Hair bangs */}
      <path
        d="M20 24 Q25 20 30 24"
        stroke="#2d3a4a"
        strokeWidth="3"
        strokeLinecap="round"
        fill="none"
...
function getBotReply(input: string, history: string[]): BotReply {
  ...
  // Greeting
  if (
    /^(hi|hello|hey|namaste|helo|hii|namaskar|hy|good morning|good evening|good afternoon)\b/.test(
      q,
    )
  ) {
    const greets = [
  ...
  if (/map|direction|navigate|rasta|kaise pahunche/.test(q))
    return {
      ...
      quick: [{ label: '📞 Call karein', value: 'contact details' }],
    };
  ...
  // Specific courses
  if (
    /industrial safety|fire safety|fire and safety|qa.?qc|quality|ndt|welding|piping|hvac|fitter/.test(
      q,
    )
  ) {
    ...
    const detail = key
      ? courseMap[key]
      : 'Yeh ek bahut popular course hai! Fees aur admission ke liye 📞 7061005611 pe call karein.';
    return {
      text: detail,
      quick: [
  ...
  const streamMsg = useCallback((id: number, fullText: string, quick?: QuickReply[]) => {
    ...
    const tick = () => {
      ...
      if (i < fullText.length) setTimeout(tick, speed);
        ...
        if (quick) {
          setMsgs((prev) => prev.map((m) => (m.id === id ? { ...m, quick } : m)));
        }
    ...
    <>
      ...
      >
        {open ? (
          ...
          <GirlBotAvatar size={40} className="rpyv2-chat-bubble-avatar" />
        )}
      ...
      {/* Panel */}
      <div
        className={`rpyv2-chat-panel${open ? ' rpyv2-chat-panel--open' : ''}`}
        role="dialog"
        aria-label="RPY Tech AI Support"
      >
        {/* Header */}
            ...
            <div key={msg.id} className={`rpyv2-chat-msg rpyv2-chat-msg--${msg.from}`}>
              ...
              <div className="rpyv2-chat-msg-wrap">
                <div className="rpyv2-chat-msg-bubble">
                  ...
                  {msg.streaming && (
                    <span className="rpyv2-chat-cursor" aria-hidden>
                      ▋
                    </span>
                  )}
                </div>
                ...
                {!msg.streaming && msg.quick && (
                  <div className="rpyv2-chat-quick">
                    {msg.quick.map((q) => (
                      <button
                        key={q.value}
                        className="rpyv2-chat-quick-btn"
