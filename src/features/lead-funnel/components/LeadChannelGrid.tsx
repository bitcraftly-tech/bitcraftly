...
interface LeadChannelGridProps {
   ...
   */
  onApplyIntent?: (
    intent: Extract<LeadIntent, "discovery" | "audit">,
  ) => void;
}

const channelButtonClassName = cn(
  "lead-funnel__channel-btn",
  "lead-funnel__channel-card-cta",
);

/**
 * Secondary contact channels beside the main lead form.
 */
export function LeadChannelGrid({
  source = "contact-page",
  onApplyIntent,
}: LeadChannelGridProps) {
  const calendlyReady = isCalendlyConfigured();

  return (
    <aside
      className="lead-funnel__channels"
      aria-labelledby="lead-channels-heading"
    >
          ...
          <div className="lead-funnel__channel-card-header">
            ...
            <div className="lead-funnel__channel-card-copy">
              ...
              <p className="lead-funnel__channel-card-meta">
                Fast chat — often same-day replies
              </p>
            </div>
          </div>
          <WhatsAppCta
