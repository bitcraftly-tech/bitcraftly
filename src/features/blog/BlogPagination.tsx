...
export function BlogPagination({ page, totalPages, category = 'all', q }: BlogPaginationProps) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    ...
    >
      {prevDisabled ? (
        <span className="font-sans text-[14px] text-muted-foreground/60">Previous</span>
      ) : (
        <Link
      ...
      {nextDisabled ? (
        <span className="font-sans text-[14px] text-muted-foreground/60">Next</span>
      ) : (
        <Link
          href={buildListingHref({ category, q, page: page + 1 })}
          className={cn(
