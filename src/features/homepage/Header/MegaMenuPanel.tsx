...
import type { NavChildLink, NavFeaturedCard, NavGroup, SiteNavLink } from '@/constants/navigation';
...
export function MegaMenuPanel({ link, menuId, onSelect, className }: MegaMenuPanelProps) {
  ...
  return (
    ...
    >
      <div
        className={cn('grid min-w-0', featured ? 'lg:grid-cols-[minmax(0,1fr)_280px]' : undefined)}
      >
            ...
            <>
              ...
              >
                {flatChildren.map((item) => (
                  <MegaMenuItem key={item.href} item={item} onSelect={onSelect} />
                ))}
              </div>
            </>
          )}
        ...
        {featured ? (
          <FeaturedMegaCard item={featured} onSelect={onSelect} compact={isCompact} />
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-center justify-between gap-[8px]">
          ...
          {item.badge ? (
            <span className="inline-flex items-center gap-[4px] rounded-full border border-primary/15 bg-primary/10 px-[8px] py-[3px] font-sans text-[10px] font-semibold text-primary">
              <Icon name="sparkles" size="sm" aria-hidden className="!h-[10px] !w-[10px]" />
              {item.badge}
            </span>
          ) : null}
        </div>
        ...
        {highlights.length > 0 ? (
          <ul
            className={cn('relative mt-[10px] flex flex-col gap-[5px]', compact && 'line-clamp-4')}
          >
            {(compact ? highlights.slice(0, 3) : highlights).map((highlight) => (
              ...
              >
                <span className="grid size-[14px] shrink-0 place-items-center rounded-full bg-primary/10 text-primary">
                  <Icon name="check" size="sm" aria-hidden className="!h-[8px] !w-[8px]" />
                </span>
