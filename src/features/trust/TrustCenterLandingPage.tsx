  ...
  return (
    <PageShell className="trust-page">
        ...
        <Container size="xl">
          <div className="trust-page__prose">
            <header className="trust-page__intro">
              <p className="trust-page__eyebrow">{TRUST_LANDING.standardsEyebrow}</p>
              <h2 id="trust-standards-heading" className="trust-page__heading">
                {TRUST_LANDING.standardsTitle}
              </h2>
              <p className="trust-page__body">{TRUST_LANDING.standardsDescription}</p>
            </header>
            ...
            {TRUST_AREAS.map((area) => (
              ...
              >
                <div className="trust-page__area-intro">
                  <h2 id={`trust-area-${area.id}`} className="trust-page__heading">
                    {area.title}
                  </h2>
                  <p className="trust-page__area-count">
                    {area.standards.length} {area.standards.length === 1 ? 'standard' : 'standards'}
                  </p>
                </div>
            ...
            <section className="trust-page__access" aria-labelledby="trust-access-heading">
              <h2 id="trust-access-heading" className="trust-page__heading">
                {TRUST_LANDING.accessTitle}
              </h2>
              <p className="trust-page__body">{TRUST_LANDING.accessDescription}</p>
              <button
                type="button"
      ...
      <MarketingFinalCtaBand
        ...
        trust={['Public standards summaries', 'Founder-led accountability', 'Secure modern stack']}
      />
    </PageShell>
  );
}
