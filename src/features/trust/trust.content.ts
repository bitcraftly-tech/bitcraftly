...
export const TRUST_LANDING = {
  ...
  standardsDescription: 'Full operational library stays behind authenticated dashboard access.',
...
export function getTrustOverviewStats() {
  const governedAreas = TRUST_AREAS.reduce((count, area) => count + area.standards.length, 0);
  const approvedPdfs = TRUST_AREAS.reduce(
    (count, area) => count + area.standards.filter((item) => item.status === 'approved').length,
    0,
  );
