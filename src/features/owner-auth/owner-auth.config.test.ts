...
const ENV_KEYS = ['OWNER_AUTH_EMAIL', 'OWNER_AUTH_PASSWORD', 'OWNER_SESSION_SECRET'] as const;
...
describe('verifyOwnerCredentials', () => {
  ...
  it('accepts matching credentials case-insensitively for email', () => {
    ...
    expect(verifyOwnerCredentials('Owner@Company.com', 'super-secret-password', config)).toBe(true);
  });
  ...
  it('rejects invalid credentials', () => {
    ...
    expect(verifyOwnerCredentials('owner@company.com', 'wrong-password', config)).toBe(false);
  });
