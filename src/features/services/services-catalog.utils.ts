...
export function buildServiceSearchIndex(card: ServiceCardModel): string {
  return [card.title, card.description, card.slug, card.bestFor ?? '', ...(card.tags ?? [])]
    .join(' ')
    .toLowerCase();
}

export function cardMatchesQuery(card: ServiceCardModel, query: string): boolean {
  if (!query) return true;
  return buildServiceSearchIndex(card).includes(query);
}

export function cardMatchesFilter(card: ServiceCardModel, filter: string | null): boolean {
