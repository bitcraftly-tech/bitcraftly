...
import { DEFAULT_AI_PROVIDER_ID, WELCOME_MESSAGE } from '../assistant.config';
...
export function useAssistantChat(options: UseAssistantChatOptions = {}): UseAssistantChatResult {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => [createWelcome(providerId)]);
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
          setMessages((prev) =>
            prev.map((message) => {
              ...
              const status: ChatMessageStatus = message.content ? 'complete' : 'error';
              return {
                ...message,
                status,
                content: message.content || 'Generation stopped before a reply was ready.',
              };
            }),
          );
        } else {
