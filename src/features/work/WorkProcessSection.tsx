...
import { WORK_PROCESS, WORK_PROCESS_COPY, WORK_PROCESS_TRUST } from './work.content';
import './work.css';
...
export function WorkProcessSection() {
  return (
    ...
    >
      ...
      <ol className="work-process__timeline" aria-label={WORK_PROCESS_COPY.timelineLabel}>
        {WORK_PROCESS.map((step, index) => {
          ...
          return (
            <li
              key={step.id}
              className={cn('work-process__step', `work-process__step--${step.tone}`)}
            >
              ...
              <article className="work-process__card work-process__glass">
                <div className="work-process__card-top">
                  <span className="work-process__icon" aria-hidden>
                    <Icon name={step.icon} size="sm" className="h-[20px] w-[20px]" />
                  </span>
                  <span
      ...
      <ul className="work-process__trust" aria-label={WORK_PROCESS_COPY.trustLabel}>
        {WORK_PROCESS_TRUST.map((item) => (
          <li key={item.id} className="work-process__trust-item work-process__glass">
            <span className="work-process__trust-icon" aria-hidden>
              <Icon name={item.icon} size="sm" className="h-[20px] w-[20px]" />
            </span>
