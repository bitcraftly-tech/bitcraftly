...
import { WORK_RESULT_HIGHLIGHTS, WORK_RESULT_KPIS, WORK_RESULTS_COPY } from './work.content';
import './work.css';

const WorkResultsDashboardVisual = dynamic(
  () => import('./WorkResultsDashboardVisual').then((mod) => mod.WorkResultsDashboardVisual),
  {
    loading: () => <div className="work-results__dash work-results__dash--skeleton" aria-hidden />,
    ssr: true,
  },
);
      ...
      <div className="work-results__panel">
        <ul className="work-results__stats" aria-label="Business KPI cards">
          {WORK_RESULT_KPIS.map((kpi) => (
            <li key={kpi.id}>
              <article className={cn('work-results__stat', `work-results__stat--${kpi.tone}`)}>
                <span className="work-results__stat-icon" aria-hidden>
                  <Icon name={kpi.icon} size="sm" className="h-[15px] w-[15px]" />
                </span>
        ...
        <aside className="work-results__visual" aria-label={WORK_RESULTS_COPY.dashboardLabel}>
          <WorkResultsDashboardVisual />
        </aside>

        <ul className="work-results__pills" aria-label={WORK_RESULTS_COPY.highlightsLabel}>
