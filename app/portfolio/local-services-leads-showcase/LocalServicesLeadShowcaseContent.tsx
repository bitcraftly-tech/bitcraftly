...
const FEATURE_STRIP = [
  { title: "Verified professionals", desc: "ID checks · skill badges · repeat-job scoring.", icon: ShieldCheck },
  { title: "Fast response", desc: "Median dispatch under 90 mins · SLA badges per zone.", icon: Clock },
  { title: "Affordable pricing", desc: "Written estimates before wrench touches bolt.", icon: BadgeIndianRupee },
  { title: "24×7 support", desc: "Night desk for emergencies · WhatsApp ticket IDs.", icon: Headphones },
  { title: "Guaranteed work", desc: "30‑day rework promise on labour · parts as per invoice.", icon: Wrench },
] as const;
...
const TESTIMONIALS = [
  {
    quote: "Quoted before they climbed the ladder — AC gas + PCB sorted same evening. Invoice matched the WhatsApp estimate.",
    name: "Priyanka Das",
  ...
  {
    quote: "Painting team masked floors obsessively. Site supervisor shared daily progress pics without me asking.",
    name: "Vikash Singh",
    role: "Row house · Adityapur",
  },
] as const;

const AREAS = ["Sakchi", "Bistupur", "Mango", "Kadma", "Sonari", "Adityapur", "Gamharia", "Nearby highways · SOS"] as const;
...
const PLANS = [
  ...
  },
  {
    ...
    bullets: ["Ideal painting prep · deep clean burst", "Tools included · debris bag-out", "Add assistant +₹799"],
    highlight: true,
  },
  {
    name: "Comfort AMC",
    ...
    <>
        ...
        <div className={`${CONTAINER} relative py-14 lg:py-20`}>
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-dark-text-secondary sm:text-lg">
              SnapFix Crew is a lead-gen landing specimen — quote-first flows, WhatsApp handoffs, and zone-aware dispatch messaging tailored for metro micro‑markets.
            </p>
            ...
            <div className="mx-auto mt-12 grid max-w-3xl grid-cols-3 gap-3 text-center text-[11px] text-dark-text-tertiary sm:text-xs">
              <div className="rounded-xl border border-dark-border-primary bg-dark-bg-card/80 py-3 backdrop-blur-sm">
                <p className="font-[var(--font-playfair)] text-xl font-semibold text-emerald-300">4.9</p>
                <p className="mt-1">Avg rating · illustrative</p>
              </div>
              <div className="rounded-xl border border-dark-border-primary bg-dark-bg-card/80 py-3 backdrop-blur-sm">
                <p className="font-[var(--font-playfair)] text-xl font-semibold text-violet-300">18 min</p>
                <p className="mt-1">Median callback · demo metric</p>
              </div>
              <div className="rounded-xl border border-dark-border-primary bg-dark-bg-card/80 py-3 backdrop-blur-sm">
                <p className="font-[var(--font-playfair)] text-xl font-semibold text-teal-300">260+</p>
      ...
      <section id="services" className={`${CONTAINER} scroll-mt-28 py-14 md:py-16`}>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-violet-400/90">Popular services</p>
            <h2 className="mt-2 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Book trusted crews</h2>
          </div>
          <p className="max-w-md text-sm text-dark-text-secondary">Category tiles tuned for thumb-scroll discovery — each expands into WhatsApp deep-links on production stacks.</p>
        </div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {POPULAR.map(({ name, blurb, icon: Icon, tone }) => (
            ...
            >
              ...
              <p className="mt-4 font-[var(--font-playfair)] text-lg font-semibold text-white">{name}</p>
              <p className="mt-2 flex-1 text-xs leading-relaxed text-white/70">{blurb}</p>
              <span className="mt-4 inline-flex text-[11px] font-semibold text-emerald-200">Tap-to-quote · mock</span>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-dark-border-primary bg-dark-bg-secondary/25 py-14 md:py-16">
        <div className={`${CONTAINER} grid gap-10 lg:grid-cols-2 lg:items-center`}>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-emerald-400/90">Why choose us</p>
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Built for conversion, not clutter</h2>
            <p className="mt-4 text-sm leading-relaxed text-dark-text-secondary">
              Operators win when enquiry friction drops — these bullets mirror integrations we wire on real deployments (CRM, telephony, WhatsApp Cloud API).
            </p>
          </div>
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Neighbours who booked again</h2>
        </div>
      ...
      <section className="border-y border-dark-border-primary bg-dark-bg-secondary/30 py-14 md:py-16">
        <div className={`${CONTAINER}`}>
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-emerald-400/90">Pricing plans</p>
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Transparent starting points</h2>
            <p className="mt-4 text-sm text-dark-text-secondary">Fictional slabs — final quote after scope photos / site visit.</p>
          </div>
          <div className="mx-auto mt-10 grid max-w-5xl gap-6 lg:grid-cols-3">
            {PLANS.map((p) => (
              ...
              >
                ...
                <p className={`mt-3 font-[var(--font-playfair)] text-xl font-semibold text-dark-text-primary ${p.highlight ? "mt-2" : ""}`}>{p.name}</p>
                <p className="mt-3 font-[var(--font-playfair)] text-3xl font-semibold text-emerald-300">{p.price}</p>
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Hyperlocal dispatch rings</h2>
          ...
          <div className="flex flex-wrap gap-2 lg:max-w-md lg:justify-end">
            {AREAS.map((a) => (
              <span key={a} className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-4 py-2 text-xs font-semibold text-emerald-100">
                {a}
              </span>
            ))}
          </div>
      ...
      {/* Booking */}
      <section id="booking" className="scroll-mt-28 border-t border-dark-border-primary bg-dark-bg-secondary/35 py-14 md:pb-20">
        <div className={`${CONTAINER}`}>
          <div className="mx-auto max-w-3xl">
            <div className="text-center">
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-emerald-400/90">Booking form</p>
              <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Tell us what broke</h2>
              <p className="mx-auto mt-4 max-w-xl text-sm text-dark-text-secondary">
                Minimal fields for maximum completions — optional AI summarisation hooks on backend builds.
              </p>
            </div>
            <div className="mt-10 rounded-2xl border border-dark-border-primary bg-dark-bg-card p-6 md:p-8">
              ...
              <p className="mt-8 text-center text-sm text-dark-text-secondary">
                Prefer voice?{" "}
                <ShowcaseAnchor href={`tel:${SUPPORT_PHONE_DISPLAY.replace(/\s/g, "")}`} className="font-semibold text-emerald-400 hover:text-emerald-300">
                  {SUPPORT_PHONE_DISPLAY}
                </ShowcaseAnchor>
              </p>
            </div>
            <p className="mx-auto mt-8 max-w-2xl text-center text-[11px] leading-relaxed text-dark-text-tertiary">
              SnapFix Crew is fictional · zones & pricing are illustrative · UI specimen by Bitcraftly only.
            </p>
          </div>
        </div>
      </section>
