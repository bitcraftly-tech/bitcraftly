...
const FEATURES = [
  { title: "Expert trainers", desc: "NSCA · ACSM lanes · injury-history onboarding dossiers.", icon: Users },
  { title: "Modern equipment", desc: "Force plates · cable rigs calibrated quarterly.", icon: Dumbbell },
  { title: "Personalized plans", desc: "Mesocycle PDF exports · wearable ingest hooks optional.", icon: Activity },
  { title: "Diet guidance", desc: "Macro baselines · kitchen swaps aligned to regional staples.", icon: Apple },
...
const TRAINERS = [
  { name: "Coach Malik Rao", role: "Head of Strength · ex-national rugby", focus: "Injury return-to-play · barbell literacy", initials: "MR", tone: "from-violet-600/35 to-slate-100" },
  { name: "Ananya Seth", role: "Performance nutrition liaison · RD advisory desk", focus: "Body recomposition · peri-workout fuel timing", initials: "AS", tone: "from-fuchsia-600/30 to-slate-100" },
  { name: "Jordan Reyes", role: "HIIT programme director · CPT Level IV", focus: "Metabolic conditioning · wearable pacing cues", initials: "JR", tone: "from-orange-500/28 to-slate-100" },
  { name: "Dr. Kiara Bose", role: "Sports physio · soft tissue diagnostics", focus: "Manual therapy · ACL graft cohort protocols", initials: "KB", tone: "from-cyan-600/22 to-slate-100" },
] as const;
...
const PLANS = [
  ...
  },
  {
    ...
    bullets: ["All programs · priority class booking", "2 PT credits monthly", "Nutrition macro audit quarterly"],
    flash: true,
  },
  {
...
const TRANSFORMATIONS = [
  { label: "12-week recomposition · pilot cohort", delta: "−6.4 kg fat · +2.1 kg lean · illustrative", tone: "from-violet-700/40 to-slate-100", weeks: "Week 0 → 12" },
  { label: "Half-marathon return · ACL-safe build", delta: "Vo₂ markers ↑18% · fictional lab export", tone: "from-fuchsia-700/35 to-slate-100", weeks: "Month 1 → 5" },
  { label: "Desk-athlete mobility sprint", delta: "Pain score ladder · posture AI screenshots weekly", tone: "from-orange-600/30 to-slate-100", weeks: "21-day burst" },
] as const;
    ...
    <>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-14">
            <div>
              ...
              <p className="mt-5 max-w-xl text-base leading-relaxed text-dark-text-secondary sm:text-lg">
                Neon-premium gym landing specimen — programming grids, trainer credibility rails, and measurable wins framed for members who crave intensity with adult supervision.
              </p>
                ...
                <div className="relative aspect-[4/5] max-h-[520px] sm:aspect-[16/13] lg:aspect-square lg:max-h-none">
                  ...
                  <div className="relative flex h-full flex-col justify-between p-6 sm:p-8">
                    ...
                    <div className="mx-auto flex flex-1 flex-col items-center justify-center">
                      ...
                      <p className="mt-6 text-center font-[var(--font-playfair)] text-lg font-semibold text-white">Athlete silhouette · cinematic rim light</p>
                      <p className="mt-2 max-w-xs text-center text-[11px] leading-relaxed text-white/45">
                        Gradient-only hero · no photography · neon fitness branding aligned with Bitcraftly dark UI
                      </p>
                    </div>
      ...
      <section id="programs" className={`${CONTAINER} scroll-mt-28 py-14 md:py-16`}>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-400/90">Fitness programs</p>
            <h2 className="mt-2 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Train with intent</h2>
          </div>
          <p className="max-w-md text-sm text-dark-text-secondary">Program tiles tuned for thumb-scroll energy — swap footage loops when you ship marketing renders.</p>
        </div>
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          {PROGRAMS.map((p) => (
            <article key={p.name} className={`overflow-hidden rounded-2xl border border-dark-border-primary bg-gradient-to-br shadow-lg ring-1 ring-fuchsia-500/10 ${p.energy}`}>
              <div className="flex min-h-[200px] flex-col justify-end border-t border-slate-200/80 bg-white/85 p-6 backdrop-blur-md">
                ...
                <p className="mt-4 font-[var(--font-playfair)] text-xl font-semibold text-white">{p.name}</p>
      ...
      <section className="border-y border-dark-border-primary bg-dark-bg-secondary/25 py-14 md:py-16">
        <div className={`${CONTAINER}`}>
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-violet-400/90">Trainers</p>
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Coaching bench depth</h2>
          </div>
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {TRAINERS.map((t) => (
              <div key={t.name} className={`overflow-hidden rounded-2xl border border-dark-border-primary bg-gradient-to-br ring-1 ring-white/5 ${t.tone}`}>
                <div className="border-t border-slate-200/80 bg-white/90 p-6 backdrop-blur-sm">
                  ...
                  <p className="mt-3 text-center text-[11px] leading-relaxed text-white/60">{t.focus}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-400/85">Membership plans</p>
          <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Pick your commitment lane</h2>
          <p className="mt-4 text-sm text-dark-text-secondary">Illustrative pricing — production stacks integrate Razorpay mandates & freeze rules.</p>
        </div>
        <div className="mx-auto mt-10 grid max-w-5xl gap-6 lg:grid-cols-3">
          {PLANS.map((plan) => (
            ...
            >
              ...
              <p className={`mt-3 font-[var(--font-playfair)] text-xl font-semibold text-dark-text-primary ${plan.flash ? "" : "mt-0"}`}>{plan.name}</p>
              <p className="mt-3 font-[var(--font-playfair)] text-3xl font-semibold text-fuchsia-300">{plan.price}</p>
      ...
      <section className="border-y border-dark-border-primary bg-dark-bg-primary py-14 md:py-16">
        <div className={`${CONTAINER} grid gap-10 lg:grid-cols-2 lg:items-start`}>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-violet-400/90">Nutrition plans</p>
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Fuel matches workload</h2>
            <p className="mt-4 text-sm leading-relaxed text-dark-text-secondary">
              Nutrition storytelling sells retention — these bullets mirror nutritionist collaboration workflows we wire on production builds.
            </p>
          </div>
      ...
      <section className={`${CONTAINER} py-14 md:py-16`}>
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-fuchsia-400/85">Client transformations</p>
            <h2 className="mt-2 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Measurable momentum</h2>
          </div>
          <p className="max-w-md text-sm text-dark-text-secondary">Before/after storytelling without stock imagery — dual-tone gradient strips evoke scan overlays.</p>
        </div>
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {TRANSFORMATIONS.map((t) => (
            <div key={t.label} className={`overflow-hidden rounded-2xl border border-dark-border-primary bg-gradient-to-br ${t.tone}`}>
              ...
              <div className="border-t border-slate-200/80 bg-white/92 p-5 backdrop-blur-sm">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-fuchsia-800">{t.weeks}</p>
      ...
      <section className="border-t border-dark-border-primary bg-dark-bg-secondary/35 py-14 md:pb-20">
        <div className={`${CONTAINER}`}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl text-dark-text-primary sm:text-4xl">Members who stayed loud</h2>
          </div>
          ...
          <p className="mx-auto mt-10 max-w-2xl text-center text-sm text-dark-text-secondary">
            ...
            <ShowcaseAnchor href={`tel:${SUPPORT_PHONE_DISPLAY.replace(/\s/g, "")}`} className="font-semibold text-fuchsia-400 hover:text-fuchsia-300">
              {SUPPORT_PHONE_DISPLAY}
            </ShowcaseAnchor>
          </p>
          <p className="mx-auto mt-6 max-w-2xl text-center text-[11px] leading-relaxed text-dark-text-tertiary">
            Apex Forge Athletic Club is fictional · metrics & pricing illustrative · UI specimen by Bitcraftly only.
          </p>
        </div>
      </section>
    </>
