...
export default function GymFitnessShowcaseDemo() {
  const { city, setTrialOpen, setPassModal, setClassModal, setCenterModal, setReelOpen, scrollToSection } = useGymDemo();
    ...
    <div>
        ...
        <div className={`${CONTAINER} grid items-center gap-10 py-12 lg:grid-cols-2 lg:py-16`}>
          <div>
            ...
            <p className="gym-text-muted mt-5 max-w-lg text-base leading-relaxed sm:text-lg">
              Group classes, gyms, sports & at-home workouts — built for your next fitness brand demo.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <button type="button" onClick={() => setTrialOpen(true)} className="gym-btn-primary inline-flex items-center gap-2 rounded-full px-7 py-3 text-sm">
                Get free trial
            ...
            <p className="gym-text-muted mt-6 flex items-center gap-2 text-xs">
              ...
              Showing centers near <span className="font-semibold text-[var(--gym-text)]">{city}</span>
            </p>
          </div>
          <div className="relative">
            ...
            <div className="gym-bg-card absolute bottom-4 left-4 right-4 rounded-xl border gym-border p-4 shadow-lg sm:left-auto sm:right-4 sm:w-56">
              <p className="text-xs font-semibold uppercase tracking-wide gym-brand-text">Live now</p>
      ...
      <section id="formats" className={`${CONTAINER} scroll-mt-24 py-14`}>
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest gym-brand-text">Group formats</p>
            <h2 className="mt-2 text-3xl font-bold">Pick your workout</h2>
          </div>
          <button type="button" onClick={() => scrollToSection("passes")} className="hidden items-center gap-1 text-sm font-semibold gym-brand-text sm:inline-flex">
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="rounded-md bg-neutral-100 px-2 py-1 text-[10px] font-semibold">{format.duration}</span>
                </div>
      ...
      <section id="passes" className="gym-bg-surface scroll-mt-24 py-14">
        <div className={CONTAINER}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="gym-text-muted mt-3 text-sm">Illustrative pricing · Razorpay-ready on production builds.</p>
          </div>
      ...
      <section id="centers" className={`${CONTAINER} scroll-mt-24 py-14`}>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          {centersToShow.map((center) => (
            <article key={center.id} className="gym-bg-card flex overflow-hidden rounded-2xl border gym-border">
              <GymLazyImage
                src={center.image}
                alt={center.name}
      ...
      <section className="gym-bg-surface py-14">
        <div className={CONTAINER}>
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-3">
            {TRAINERS.map((t) => (
              <div key={t.id} className="gym-bg-card overflow-hidden rounded-2xl border gym-border text-center">
                <GymLazyImage src={t.image} alt={t.name} wrapperClassName="aspect-square w-full" fallbackSeed={t.id} />
                <div className="p-4">
        ...
        <div className={`${CONTAINER} grid gap-6 lg:grid-cols-2 lg:items-stretch`}>
          <article className="gym-bg-card relative flex h-full min-h-[340px] flex-col overflow-hidden rounded-2xl border gym-border p-6 md:p-8">
            <div
              ...
              aria-hidden
            />
            <p className="relative text-xs font-bold uppercase tracking-widest gym-brand-text">Mobile app</p>
            <Smartphone className="relative mt-4 h-10 w-10 gym-brand-text" aria-hidden />
            <h2 className="relative mt-3 text-2xl font-bold leading-tight">Train anywhere with the FitRally app</h2>
            <p className="gym-text-muted relative mt-2 text-sm leading-relaxed">
              Full app experience on production builds — sync workouts, book classes, track progress.
            </p>
            <ul className="relative mt-6 flex flex-1 flex-col justify-center gap-3">
              {APP_FEATURES.map(({ icon: Icon, text }) => (
                <li key={text} className="flex items-center gap-3 rounded-xl border gym-border bg-[var(--gym-surface)] px-4 py-3 text-sm font-medium">
