...
const DEAL_ROWS = [
  ...
  },
  {
    ...
    products: [
      ...
      {
        title: "ShopKart Basics Microfiber Bed Sheet Set · Queen",
        price: 1299,
        list: 2499,
        rating: 4.2,
        count: 4521,
...
function ProductCard({ product }: { product: (typeof DEAL_ROWS)[number]["products"][number] }) {
  ...
  return (
    <article className="flex h-full min-w-[200px] max-w-[240px] shrink-0 flex-col rounded border border-[#ddd] bg-white p-3 shadow-sm sm:min-w-[220px]">
      ...
      <h3 className="mt-3 line-clamp-2 text-sm text-[#0f1111] hover:text-[#c7511f] hover:underline">{product.title}</h3>
      <div className="mt-1">
      ...
      {product.prime ? (
        <p className="mt-1 text-xs font-bold text-[#007185]">
          <span className="rounded bg-[#00a8e1] px-1 py-0.5 text-[10px] font-bold text-white">plus</span>
        </p>
      ) : null}
...
export default function EcommerceStoreShowcaseContent() {
  return (
    <div className="bg-[#eaeded] text-[#0f1111]">
      ...
      <section className={`${CONTAINER} py-4 md:py-5`}>
        <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
          ...
          >
            ...
            <h2 className="mt-2 text-2xl font-bold md:text-3xl">Up to 60% off | Deals on electronics</h2>
      ...
      {/* Deal carousels */}
      {DEAL_ROWS.map((row) => (
        <section key={row.title} id={row.title === "Top deals" ? "deals" : "catalog"} className={`${CONTAINER} scroll-mt-36 pb-4`}>
      ...
      <section className={`${CONTAINER} scroll-mt-36 pb-8`}>
        <div className="rounded-sm bg-white p-4 shadow-sm md:p-5">
          <p className="text-sm text-[#565959]">
            1–16 of over 10,000 results for <span className="font-bold text-[#c7511f]">wireless headphones</span>
          </p>
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {DEAL_ROWS[0].products.map((p) => (
              <article key={`plp-${p.title}`} className="flex flex-col border border-[#ddd] bg-white p-3">
                <div className={`aspect-square rounded-sm ${p.tone}`} />
                <h3 className="mt-3 line-clamp-2 text-sm hover:text-[#c7511f] hover:underline">{p.title}</h3>
      ...
      <section className={`${CONTAINER} pb-8`}>
        <div className="flex flex-col items-center justify-between gap-4 rounded-sm bg-gradient-to-r from-[#1a98ff] to-[#232f3e] px-6 py-8 text-white md:flex-row">
          <div>
            <p className="text-lg font-bold">Try ShopKart Plus</p>
            <p className="mt-1 max-w-lg text-sm text-[#ddd]">
              Fast, FREE delivery · exclusive deals · streaming preview strip — membership UX like leading marketplaces.
            </p>
          </div>
          <ShowcaseLink
      ...
      <p className={`${CONTAINER} pb-6 text-center text-[11px] text-[#565959]`}>
        ShopKart is fictional · marketplace UI specimen · not affiliated with any retailer · by Bitcraftly
      </p>
    </div>
  );
}
