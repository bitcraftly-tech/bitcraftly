...
export function SchoolLazyImage({
  ...
  if (eager) {
    return (
      <span className={wrapClass(wrapperClassName)}>
        <img src={currentSrc} alt={alt} className={className} loading="eager" decoding="async" onError={handleError} />
      </span>
    );
  }
  ...
  return (
    <span ref={containerRef} className={wrapClass(wrapperClassName)}>
      {showImage && !loaded ? <span className="school-image-skeleton absolute inset-0 z-0 block" aria-hidden /> : null}
      {showImage ? (
        <img
          ref={imgRef}
          src={currentSrc}
