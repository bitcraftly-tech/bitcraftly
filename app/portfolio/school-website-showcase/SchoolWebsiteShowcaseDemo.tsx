        ...
        <div className="grid gap-8 lg:grid-cols-3 lg:gap-10">
          ...
          <div>
            ...
            <p className="school-text-muted mt-4 text-sm leading-relaxed">
              From playgroup to grade XII, we blend academic rigour with arts, sports and leadership — nurturing confident,
              compassionate global citizens on a world-class campus.
            </p>
            <ul className="mt-5 space-y-2.5">
              {ABOUT_FEATURES.map((f) => (
                <li key={f} className="flex items-center gap-2 text-sm font-medium text-[var(--school-text)]">
          ...
          <div className="school-vm-panel rounded-2xl border school-border p-5 lg:p-6">
            {VISION_MISSION.map((v) => {
              ...
              return (
                <div key={v.id} className="flex gap-4 border-b school-border py-4 first:pt-0 last:border-0 last:pb-0">
                  <Icon className="mt-0.5 h-5 w-5 shrink-0 text-[var(--school-orange)]" aria-hidden />
                  <div>
