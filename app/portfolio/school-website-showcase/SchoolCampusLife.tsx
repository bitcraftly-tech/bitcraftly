...
function ReelCard({ item, index }: { item: CampusItem; index: number }) {
  ...
  return (
    <article
      {...schoolCardClickProps(() => setLightbox({ src: item.image, title: item.title, alt: item.title }))}
    ...
    >
      ...
      <div className="rounded-b-2xl border border-t-0 border-white/20 bg-white px-4 py-3.5 shadow-lg">
        ...
        <p className="mt-1 text-sm font-bold leading-snug text-[var(--school-navy)]">{item.title}</p>
      </div>
    </article>
  );
}
      ...
      <div className={`${CONTAINER} py-14 lg:py-16`}>
        <div className="lg:grid lg:grid-cols-[minmax(0,340px)_minmax(0,1fr)] lg:gap-10 xl:grid-cols-[380px_1fr]">
          <div className="text-white lg:py-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--school-orange)]">Campus Life</p>
            <h2 className="mt-3 font-[var(--font-playfair)] text-3xl font-bold leading-tight sm:text-4xl">
              Where Learning Meets Life
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-white/80">
              Swipe through sports, labs, arts and celebrations — a day at Elevate is never just classrooms.
            </p>
          ...
          <div className="relative mt-10 min-w-0 lg:mt-0">
            <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-[var(--school-navy)] to-transparent" aria-hidden />
            <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-12 bg-gradient-to-l from-[var(--school-navy)] to-transparent" aria-hidden />

            <div ref={scrollerRef} className="school-campus-reel flex gap-4 overflow-x-auto pb-2 pt-1">
