  ...
  const programCards = ACADEMIC_PROGRAMS.map((p) => (
    ...
    >
      <div className="relative">
        <SchoolLazyImage src={p.image} alt={p.title} wrapperClassName="aspect-[16/10] w-full" fallbackSeed={p.id} />
      </div>
      <div className="relative px-4 pb-4 pt-6">
        ...
        <p className="text-[11px] font-bold uppercase tracking-wide text-[var(--school-orange)]">{p.grades}</p>
    ...
    <div className="relative mt-10">
      ...
      <div ref={viewportRef} className="overflow-hidden px-8 sm:px-10">
        <motion.div
          ...
          initial={false}
          animate={{ x: offset }}
          transition={reduceMotion ? { duration: 0 } : { type: "tween", duration: 0.75, ease: [0.32, 0.72, 0, 1] }}
        >
          {programCards}
        </motion.div>
      </div>
